...
def is_point_in_box(point, xyxy):
    x1, y1, x2, y2 = [int(xy) for xy in xyxy]
    X, Y = point[0], point[1]
    return x1 <= X <= x2 and y1 <= Y <= y2


def is_obj_present(xyxy, thresh=DISTANCE_THRESHOLD):
    for i in range(MATRIX_SIZE_X):
        for j in range(MATRIX_SIZE_Y):
            if dist_and_coord[i][j][2] >= thresh and is_point_in_box(
                    dist_and_coord[i][j], xyxy):
                return True
    return False


def dist_and_coordinates(size):
    l, h = size[1], size[0]
    for i in range(4):
...
class Inferer:
    def __init__(self, weights, device, yaml, img_size, half, conf_thres,
                 iou_thres, classes, agnostic_nms, max_det):
    ...
    def infer(self, img, img_src):
        ...
        gn = torch.tensor(img_src.shape)[[1, 0, 1,
        ...
        if len(det):
            det[:, :4] = self.rescale(img.shape[2:], det[:, :4],
                                      img_src.shape).round()
    ...
    def check_img_size(self, img_size, s=32, floor=0):
        ...
        if isinstance(img_size, int):  # integer i.e. img_size=640
            ...
            new_size = [
                max(self.make_divisible(x, int(s)), floor) for x in img_size
            ]
        else:
    ...
    @staticmethod
    def font_check(font=yolov6_path + '/yolov6/utils/Arial.ttf', size=10):
...
args = {
    "weights": yolov6_path +
    "/weights/yolov6n.pt",  # Path to weights file default weights are for nano model
    "source":
    "./img.jpg",  #Path to image file or it can be a directory of image
    "yaml": yolov6_path + "/data/coco.yaml",
    "img-size": 640,  # default image size
    "conf-thres": 0.25,  # confidence threshold for inference.
    "iou-thres": 0.45,  # NMS IoU threshold for inference.
    "max-det": 1000,  # maximal inferences per image
    "device": 0,  # device to run our model i.e. 0 or 0,1,2,3 or cpu
    "save-img": True,  # save visuallized inference results.
...
inferer = Inferer(weights=args['weights'],
                  device=args['device'],
                  yaml=args['yaml'],
...
start = time.time()
img, img_src = Inferer.precess_image(img_src, inferer.img_size,
                                     inferer.model.stride, args['half'])
det = inferer.infer(img, img_src)
end = time.time() - start
fps_txt = "{:.2f}".format(1 / end)

dist_and_coordinates(img_src.shape)
for *xyxy, conf, cls in reversed(det):
    if is_obj_present(xyxy):
        class_num = int(cls)  # integer class
        label = None if args['hide-labels'] else (
            inferer.class_names[class_num] if args['hide-conf'] else
            f'{inferer.class_names[class_num]} {conf:.2f}')
        Inferer.plot_box_and_label(img_src,
                                   max(round(sum(img_src.shape) / 2 * 0.003),
                                       2),
                                   xyxy,
                                   label,
